...
void main() {
  group('encodeFragments', () {
    ...
    test(
      'a message one byte over fragment payload spills to a second fragment',
      () {
    ...
    test(
      'a 2 KiB message fragments and reassembles to bytes-identical JSON',
      () {
    ...
    test(
      'a custom maxFragmentSize splits at that boundary and reassembles',
      () {
    ...
    test(
      'maxFragmentSize at the BLE floor still produces well-formed fragments',
      () {
        ...
        final fragments = encodeFragments(
          json,
          maxFragmentSize: kMinFragmentSize,
        );
    ...
    test(
      'multibyte UTF-8 sequences reassemble correctly across fragment boundaries',
      () {
  ...
  group('FragmentReassembler', () {
    test(
      'returns null on intermediate fragments and the message on the last',
      () {
    ...
    test(
      'fragment payload that overshoots the declared length is rejected',
      () {
    ...
    test(
      'after a violation, the reassembler is reset and accepts a new message',
      () {
        final r = FragmentReassembler();
        try {
          r.feed(Uint8List.fromList([0x00, 0x00]));
