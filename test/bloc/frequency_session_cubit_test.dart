...
void main() {
  group('FrequencySessionCubit', () {
    ...
    blocTest<FrequencySessionCubit, FrequencySessionState>(
      ...
      build: () => _makeCubit(identityStore: _FakeStore()..throwOnGet = true),
      act: (cubit) => cubit.bootstrap(),
      expect: () => [const SessionOnboarding()],
    );
    ...
    blocTest<FrequencySessionCubit, FrequencySessionState>(
      ...
      build: () => _makeCubit(identityStore: _FakeStore()..throwOnSet = true),
      seed: () => const SessionOnboarding(),
    ...
    blocTest<FrequencySessionCubit, FrequencySessionState>(
      ...
      expect: () => [
        const SessionRoom(
          ...
          roster: [ProtocolPeer(peerId: 'fake-peer-id', displayName: 'Maya')],
        ),
      ],
    );
    ...
    test('joinRoom as host calls startAdvertising and startGattServer on audio '
        'with the minted sessionUuid + display name', () async {
      ...
      expect(calls.where((c) => c.method == 'startGattServer'), hasLength(1));
    ...
    test('joinRoom as host bails out when peerId resolution fails — '
        'no broken room is emitted', () async {
    ...
    test(
      ...
      () async {
        ...
        expect(
          methods,
          containsAll(<String>{'stopAdvertising', 'stopGattServer'}),
        );
    ...
    test('close() while in a host room tears down host BLE surfaces', () async {
    ...
    test(
      '_promoteToHost: JoinRequest during stopVoiceTransport window still '
      'carries the voice PSM (not null) — #120',
      () async {
        TestWidgetsFlutterBinding.ensureInitialized();
        const channel = MethodChannel('com.elodin.walkie_talkie/audio');
        // Hold stopVoiceTransport() open so the promote sits in the awaited
        // window where, pre-fix, _voiceServerStartup was still null and a
        // JoinRequest got JoinAccepted(voicePsm: null).
        final stopGate = Completer<void>();
        TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
            .setMockMethodCallHandler(channel, (call) async {
              if (call.method == 'stopVoiceTransport') {
        ...
        // Promote peer-2 to host. The composite voice-server startup begins
        // and blocks inside stopVoiceTransport() (stopGate not yet completed).
        for (final frag in encodeFragments(
          const HostTransfer(
            peerId: 'peer-1',
            seq: 1,
        ...
        // A guest joins *while* stopVoiceTransport is still pending — the
        // live-host window the fix protects.
        for (final frag in encodeFragments(
          const JoinRequest(
            peerId: 'peer-3',
            seq: 1,
        ...
        // Now release the window: stop completes, startVoiceServer resolves.
        stopGate.complete();
        await Future<void>.delayed(Duration.zero);
        await Future<void>.delayed(Duration.zero);
        await Future<void>.delayed(Duration.zero);
    ...
    test('joinRoom as guest does NOT record the freq', () async {
    ...
    test('joinRoom as host with existingSessionUuid reuses it instead of '
        'minting a fresh one (#219 Resume path)', () async {
    ...
    test('joinRoom as host swallows record() failures', () async {
      ...
      await expectLater(cubit.joinRoom(freq: '104.3', isHost: true), completes);
      await Future<void>.delayed(Duration.zero);
      expect(cubit.state, isA<SessionRoom>());
    ...
    test(
      ...
      () async {
        ...
        expect(s.recentHostedFrequencies, [
          ...
          const RecentFrequency(freq: '100.1', nickname: 'Family channel'),
        ]);
    ...
    test('setRecentNickname with null clears an existing nickname', () async {
      final recent = _FakeRecentFrequenciesStore(
        detailed: const [
    ...
    test('setRecentNickname is a no-op outside Discovery', () async {
      final recent = _FakeRecentFrequenciesStore();
      final cubit = _makeCubit(recentFrequenciesStore: recent);
      cubit.emit(
        const SessionRoom(myName: 'Maya', roomFreq: '104.3', roomIsHost: true),
    ...
    test(
      ...
      () async {
        ...
        await expectLater(cubit.setRecentPinned('100.1', true), completes);
    ...
    test('setRecentPinned is a no-op outside Discovery', () async {
      final recent = _FakeRecentFrequenciesStore();
      final cubit = _makeCubit(recentFrequenciesStore: recent);
      cubit.emit(
        const SessionRoom(myName: 'Maya', roomFreq: '104.3', roomIsHost: true),
    ...
    test('deleteRecentFrequency is a no-op outside Discovery', () async {
      final recent = _FakeRecentFrequenciesStore();
      final cubit = _makeCubit(recentFrequenciesStore: recent);
      cubit.emit(
        const SessionRoom(myName: 'Maya', roomFreq: '104.3', roomIsHost: true),
    ...
    test(
      ...
      () async {
        ...
        await expectLater(cubit.deleteRecentFrequency('100.1'), completes);
    ...
    test(
      'sendMediaCommand emits the originator command on the stream',
      () async {
        final cubit = _makeCubit();
        cubit.emit(
          const SessionRoom(
    ...
    test(
      'sendMediaCommand suspended through close is a silent no-op',
      () async {
  ...
  group('notifyDrop / ConnectionPhase', () {
    ...
    test('notifyDrop on host room is a no-op', () async {
      ...
      cubit.emit(
        const SessionRoom(myName: 'Devon', roomFreq: '104.3', roomIsHost: true),
      );
  ...
  group('Heartbeats', () {
    ...
    test('joinRoom starts the heartbeat scheduler when a transport is wired; '
        'leaveRoom stops it', () async {
      final t = makeTestTransport();
      final scheduler = HeartbeatScheduler(
        pingInterval: const Duration(hours: 1),
        missThreshold: const Duration(seconds: 60),
      );
      final cubit = _makeCubit(heartbeats: scheduler, transport: t.transport);
      cubit.emit(const SessionDiscovery(myName: 'Maya'));
    ...
    test('inbound non-Heartbeat messages also refresh the watermark', () async {
    ...
    test('guest: a non-host peer-lost is ignored', () async {
      final t = makeTestTransport();
      var fakeNow = DateTime(2026, 1, 1, 12, 0, 0);
      final scheduler = HeartbeatScheduler(
        pingInterval: const Duration(hours: 1),
  ...
  group('SignalReport', () {
    ...
    test('guest joinRoom with audio + transport starts the SignalReporter; '
        'leaveRoom stops it', () async {
      final t = makeTestTransport();
      ...
      await cubit.joinRoom(freq: '104.3', isHost: false, macAddress: 'AA:BB');
    ...
    test(
      'guest joinRoom without audio does NOT start the SignalReporter',
      () async {
    ...
    test(
      'host suppresses toast for a neighbor that is not in the roster',
      () async {
        ...
        Future<void> feedReport(int seq) async {
          final json = SignalReport(
            ...
            seq: seq,
            atMs: seq * 1000,
            neighbors: [const NeighborSignal(peerId: 'p-ghost', rssi: -90)],
          ).encode();
          for (final frag in encodeFragments(json)) {
            t.inbox.add((endpointId: 'AA:BB', bytes: frag));
          }
    ...
    test(
      'host ignores inbound SignalReports when local role is guest',
      () async {
  ...
  group('Permission revocation', () {
    ...
    test('recover with no display name routes back to Onboarding', () async {
      final watcher = _FakePermissionWatcher();
      final cubit = _makeCubit(permissionWatcher: watcher);
      ...
      cubit.emit(
        SessionPermissionDenied(missing: const [AppPermission.microphone]),
      );
      await cubit.bootstrap();
    ...
    test('changing missing list while denied refreshes the screen', () async {
      final watcher = _FakePermissionWatcher();
      final cubit = _makeCubit(
        identityStore: _FakeStore(initial: 'Maya'),
        permissionWatcher: watcher,
    ...
    test('recheckPermissions delegates to the watcher', () async {
      final watcher = _FakePermissionWatcher();
      final cubit = _makeCubit(permissionWatcher: watcher);
      await cubit.bootstrap();
    ...
    test('recheckPermissions is a no-op when no watcher is wired', () async {
      final cubit = _makeCubit();
      await cubit.bootstrap();
    ...
    test('recover-from-denied does not clobber a fresh denied state during the '
        'recent-frequencies disk-read await', () async {
  ...
  group('TalkingState outbound', () {
    ...
    test('rapid alternating VAD edges produce TalkingStates with strictly '
        'increasing seq numbers on the wire', () async {
      final t = makeTestTransport();
    ...
    test('a slow in-flight send blocks subsequent VAD edges from racing onto '
        'the wire', () async {
    ...
    test('transport-level send failures do not poison the chain — later '
        'edges still reach the wire', () async {
    ...
    group('host handover', () {
      ...
      test('receiving HostTransfer as a non-promoted guest updates hostPeerId '
          'and enters reconnecting', () async {
        final t = makeT();
      ...
      test('Leave from old host is not a room-kill after HostTransfer '
          'updates hostPeerId', () async {
